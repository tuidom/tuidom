...
- **«Переполнение» реализуется реперентингом, не рисованием за границами.** Попап,
  ...
  контекстные меню, оверлеи хоста. Внутри слоя дети тоже вложены — слой раскладывает их
...
## Хит-тест: один источник порядка
...
**Подключение к дереву** (аналог DOM connectedCallback): «прикреплён к родителю» ≠
...
Инварианты дерева проверяет `validateTree` (`packages/core/src/dom/validateTree.ts`): в тестах — после
каждого кадра (`TestApp`), в приложении — хост включает
`TuiApplication.validateTreeAfterRender` (vexx делает это по env
`VEXX_VALIDATE_TREE=1`; см. [TODO/EnvVarNaming.md](TODO/EnvVarNaming.md)). Помимо топологии
...
Листовые элементы (`BoxElement`, редакторный элемент Vexx) используют базовый `performLayout`, который просто применяет `constraints.constrain()` к текущему размеру. Самосайзящиеся (`PopupMenuElement`, `CompletionListElement`) переопределяют его по конвенции контракта: natural-размер из intrinsic-методов, заклампленный в constraints.
...
`cell` — частичный патч (`CellPatch`): заданные поля перезаписываются, опущенные остаются прежними. Это нужно проходам-подсветкам (кладут один `bg`, не трогая глиф) и пост-обработке через `getCell`. Обратная сторона — **непрозрачная запись ячейки обязана задавать `style`**: иначе ячейка донашивает атрибуты того, кто рисовал здесь раньше в этом же кадре, и оверлей поверх подчёркнутого текста проступает чужой волной сквозь свой фон и рамку. Штатные непрозрачные художники — `drawText`, `drawBox` и заливка собственного фона элемента (`paintOwnBackground`) — кладут `style` сами (`StyleFlags.None`, если вызывающий ничего не передал); свой цикл `setCell` с `char` пусть делает так же.
...
(`packages/core/src/dom/borderStyle.ts`) — так места, где рамка входит в геометрию,
...
`SashElement` (`packages/elements/src/sash/sashElement.ts`) — невидимый хит-таргет на границе панелей;
