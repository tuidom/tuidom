...
export function validateTree(root: TUIElement): string[] {
    ...
    if (expectedRoot !== root) {
        violations.push(
            `корень обхода ${describe(root)} не считает себя корнем: getRoot() → ${describeOrNull(expectedRoot)}`,
        );
    }
    ...
    while (stack.length > 0) {
        ...
        for (const child of node.getChildren()) {
            if (visited.has(child)) {
                violations.push(
                    `${describe(child)} встречается в дереве дважды (второй раз — как ребёнок ${describe(node)})`,
                );
            ...
            if (childRoot !== root) {
                violations.push(
                    `${describe(child)} не укоренён: getRoot() → ${describeOrNull(childRoot)} (ожидался ${describe(root)})`,
                );
            }
