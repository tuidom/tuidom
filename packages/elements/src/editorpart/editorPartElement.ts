...
export class EditorPartElement extends TUIElement {
    ...
    private handleSashDrag(index: number, boundaryScreen: number): void {
        const sizes = this.currentSizes();
        if (sizes === null) return;
        const origin = this.orientationValue === "columns" ? this.globalPosition.x : this.globalPosition.y;
        let offset = 0;
    ...
    protected override performLayout(constraints: BoxConstraints): Size {
        ...
        let offset = 0;
        for (const [i, view] of this.views.entries()) {
            const size = columns ? new Size(sizes[i], containerSize.height) : new Size(containerSize.width, sizes[i]);
