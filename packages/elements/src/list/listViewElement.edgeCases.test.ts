...
describe("ListViewElement edge cases", () => {
    ...
    it("hover state survives rows disappearing under the pointer", () => {
        ...
        list.dispatchEvent(
            new TUIMouseEvent("mousemove", { button: "none", localX: 0, localY: 4, screenX: 0, screenY: 4 }),
        );
        for (let i = 0; i < 4; i++) list.setRowHidden(`r${i}`, true);
        app.render(); // hover-индекс за пределами проекции — рендер не падает

        list.dispatchEvent(
            new TUIMouseEvent("mouseleave", { button: "none", localX: 0, localY: 0, screenX: 0, screenY: 0 }),
        );
        app.render();
